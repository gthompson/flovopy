...
def compare_asl_sources(asl1, asl2, atol=1e-8, rtol=1e-5):
    """
    Compare the 'source' dicts from two ASL objects.
    Returns True if identical, else a dict of differences.
    """
    import numpy as np
    ...
    if k1 != k2:
        diffs["key_mismatch"] = {"only_in_1": k1 - k2, "only_in_2": k2 - k1}
        return diffs

    for k in k1:
        v1, v2 = s1[k], s2[k]

        # convert UTCDateTime lists to float timestamps for comparison
        def to_arr(v):
        ...
        if np.issubdtype(a1.dtype, np.number) and np.issubdtype(a2.dtype, np.number):
            if not np.allclose(a1, a2, atol=atol, rtol=rtol, equal_nan=True):
                diffs[k] = "Numeric mismatch (not allclose)"
        else:
            if not (a1 == a2).all():
