...
def load_volcanoes(volcano_csv):
    """Load volcano list from CSV, or default to Soufriere Hills."""
...
def load_catalog(catalog_file, catalog_csv):
    """Load seismic catalog from QuakeML or CSV."""
...
def filter_catalog(df, volcano, only_one_volcano):
    """Filter catalog to events within radius around volcano."""
    if only_one_volcano:
        return df.copy()
    lat0, lon0, radius = volcano['lat'], volcano['lon'], volcano['radius_km'] * 1000
    ...
    return df[df.apply(within_radius, axis=1)].copy()
...
def plot_sausage(
    ...
    """
    Generate sausage plot of seismic activity.

    Parameters
    ----------
    df : pd.DataFrame
        Event catalog with 'time', 'latitude', 'longitude', 'magnitude'.
    ...
    binsize : str
        Bin size ('W', '2W', 'M', etc.).
    months_back : int
        How many months back to plot.
    months_per_string : int
        How many months per sausage string.
    scale : float
        Bubble size scaling.
    color_by : str
        Color sausages by 'magnitude' or 'energy'.
    title : str
        Plot title.
    """
    ...
    for idx, v in volcanoes.iterrows():
        name = v['place']
        df_volcano = filter_catalog(df, v, only_one_volcano)
        if df_volcano.empty:
            continue

        df_volcano['subclass'] = df_volcano['subclass'].fillna('all')

        binned_all = bin_events(df_volcano, interval=binsize, groupby='subclass')
        hist_counts = binned_all.groupby('time').size()

        PERCENTILES[name] = np.percentile(hist_counts.values, np.arange(101))

        dfv_recent = df_recent.copy() if only_one_volcano else filter_catalog(df_recent, v, False)
        if dfv_recent.empty:
            continue
        ...
        if color_by == "magnitude":
            # Convert each magnitude to energy
            energies = 10 ** (1.5 * binned_recent['cumulative_magnitude'] + 4.8)
            energy_by_bin = binned_recent.copy()
            energy_by_bin['energy'] = energies

            # Sum energy per bin
            summed_energy = energy_by_bin.groupby('time')['energy'].sum()

            # Convert total energy back to equivalent magnitude
            recent_mags = (np.log10(summed_energy) - 4.8) / 1.5

        elif color_by == "energy":
            # Convert each magnitude to energy and sum directly
            energies = 10 ** (1.5 * binned_recent['cumulative_magnitude'] + 4.8)
            recent_mags = energies.groupby(binned_recent['time']).sum()
        else:
        ...
        for bin_idx, (bin_time, count) in enumerate(recent_counts.items()):
            ...
            if only_one_volcano:
                x = bin_idx // (months_per_string)
                y = bin_idx % (months_per_string)
            else:
                x = idx
                y = bin_idx

            ax.scatter(x, months_per_string - y, s=msize, color=color, edgecolor='k', marker='s', zorder=3)
            scatter_times.append(bin_time.strftime("%Y-%m-%d"))

    # Axis formatting
    if only_one_volcano:
        nstrings = (months_back + months_per_string - 1) // months_per_string
        ax.set_xticks(range(nstrings))
        ax.set_xticklabels([f"{i+1}" for i in range(nstrings)])
        ax.set_xlabel(f"Block ({months_per_string} months each)")
    else:
        ax.set_xticks(range(len(volcanoes)))
        ax.set_xticklabels(volcanoes['place'], rotation=45, ha='right')
        ax.set_xlabel('Volcano')

    ax.set_ylabel(f"Months within Block")
    ax.invert_yaxis()
    ax.set_title(title)
...
def main():
    ...
    scale = 40
    color_by = "magnitude"
    title = "Sausage Plot: Seismic Activity"
    # ----------------

    volcanoes = load_volcanoes(volcano_csv)
    catalog = load_catalog(catalog_file, catalog_csv)
    ...
    plot_sausage(
        ...
        months_per_string=months_per_string,
        scale=scale,
        color_by=color_by,
        title=title,
    )
